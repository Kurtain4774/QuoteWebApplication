...
export function ThemeProvider({ children }) {
  ...
  const [customBg, setCustomBgState]     = useState(() => localStorage.getItem('customBg')   || '#ffffff');
  const [customText, setCustomTextState] = useState(() => localStorage.getItem('customText') || '#111827');
  ...
  function applyPreferences(prefs) {
    ...
    if ('theme'      in prefs) { setThemeState(prefs.theme);           localStorage.setItem('theme',      prefs.theme);      }
    if ('customBg'   in prefs) { setCustomBgState(prefs.customBg);     localStorage.setItem('customBg',   prefs.customBg);   }
    if ('customText' in prefs) { setCustomTextState(prefs.customText); localStorage.setItem('customText', prefs.customText); }
    if ('fontSize'   in prefs) { setFontSizeState(prefs.fontSize);     localStorage.setItem('fontSize',   prefs.fontSize);   }
  }

  return (
    <ThemeContext.Provider value={{ theme, setTheme, customBg, setCustomBg, customText, setCustomText, fontSize, setFontSize, resetTheme, applyPreferences }}>
      {children}
